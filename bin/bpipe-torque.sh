...
make_pbs_script () {
   ...
   case $QUEUE in
      batch) if [[ -z $MEMORY ]]; then
                memory_request="#PBS -l mem=${DEFAULT_BATCH_MEM}gb"
             else
...
stop () {
   ...
   if [[ $# -ge 1 ]]
      then
         # try to stop it
         module load moab #canceljob hack
	 canceljob "$1" #canceljob hack
         #qdel "$1"
         qdel_success=$?
         if [[ qdel_success == 0 ]]
            then
   ...
   if [[ $# -ge 1 ]]
	 ...
	 qstat_output=`qstat -u $USER | grep "$1"`
         ...
         if [[ $qstat_success == 0 ]]
	      ...
	      # HACK TO GET JOBID status
	      job_state=`echo $qstat_output | awk '{print $10}'`
	      #job_state=`echo "$qstat_output" | awk '/job_state =/ { print $3 }'`
               case "$job_state" in
                  ...
                  # XXX what to do if the awk fails
                  C) command_exit_status=$job_state
